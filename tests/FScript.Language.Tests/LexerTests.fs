...
type LexerTests () =
    ...
    [<Test>]
    member _.``Tokenizes raise keyword`` () =
        let tokens = Lexer.tokenize "raise \"boom\""
        tokens |> List.exists (fun t -> t.Kind = Raise) |> should equal true

    [<Test>]
    member _.``Tokenizes and keyword`` () =
        let tokens = Lexer.tokenize "let rec f x = x and g y = y"
        tokens |> List.exists (fun t -> t.Kind = And) |> should equal true

    [<Test>]
    member _.``Tokenizes export as identifier`` () =
        let tokens = Lexer.tokenize "export let f x = x"
        tokens |> List.exists (fun t -> t.Kind = Ident "export") |> should equal true
